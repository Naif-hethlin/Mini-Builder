...
export function Sidebar() {
  ...
  return (
    <aside className="flex h-full w-full flex-col overflow-hidden border-r border-slate-200 bg-white">
      {/* Tab bar */}
      ...
      <div className="flex-1 overflow-auto p-3">
        <p className="mb-3 px-1 text-xs text-slate-500">{active.description}</p>
...
function SectionTile({
  ...
  return (
    <button
      ...
      className="group rounded-lg border border-slate-200 bg-white p-2 text-left transition-all hover:border-brand hover:shadow-sm focus-visible:border-brand focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand focus-visible:outline-offset-1"
    >
      <Thumbnail />
      <div className="mt-2 px-0.5">
        <p className="text-sm font-medium text-slate-900 group-hover:text-brand">
          {preset.label}
        </p>
...
function ComingSoon({ phase, label }: { phase: number; label: string }) {
  return (
    <div className="rounded-lg border-2 border-dashed border-slate-200 p-6 text-center">
      ...
      <p className="mt-1 text-xs text-slate-500">Coming in Phase {phase}.</p>
    </div>
  );
}
